  ...
  #programming-problem(
    ...
    #solution(
      ...
      #cetz.canvas(
        {
          ...
          let nodes = ("0": (0, 0), "1": (3, 0), "2": (1.5, -2))

          let r = 0.37

          let edges = (("0", "1", "2", "north"), ("0", "2", "3", "west"), ("1", "2", "-2", "east"),)
      ...
      #table(
        ...
        [0],
        [2],
        [min(3, 2+(−2)) = *0*... too late],
        [3],
        [2 already settled with 3],
        [0],
        [2],
        [*3* ✗],
      )
...
#definition(title: "Dynamic Ordered set")[
  Main dynamic set $S$ supporting the following operations:

  / Search(k): Return element $x$ such that $x."key" = k$, otherwise null.
  / Insert(x): Add $x$ to $S$ (assume x is not already in $S$).
  / Delete(x): Remove $x$ from $S$ (assume x is in $S$).
]

=== Applications of dynamic ordered sets
- Dictionaries
- Indexes
- File systems
- Databases
...
#definition(title: "Binary Tree")[
  - Rooted tryy
  - Each internal has a left vhild and or a right child
]

#definition(title: "Binary Search Tree")[
  - All nodes store a Key.
  - Keys are in symmetric order
]

#definition(title: "Symmetric Order")[
...
=== Storage of binary search trees
Each node $x$ stores:
/ $x."key"$: the key of the node
/ $x."left"$: pointer to the left child (null if no left child)
/ $x."right"$: pointer to the right child (null if no right child)
/ $x."parent"$: pointer to the parent node (null if root)
/ $x."Data"$: the data associated with the key (optional, can be stored in the key itself or separately)

Space: $O(n)$ for $n$ nodes in the tree.

=== Insert operation in a binary search tree
To insert a key $k$ into a binary search tree $T$:
1. Create a new node $z$ with $z."key" = k$ and $z."left" = z."right" = "null"$.
2. Start at the root of the tree and initialize a pointer $y$ to null and a pointer $x$ to the root.
3. While $x$ is not null:
  a. Set $y$ to $x$.
  b. If $z."key" < x."key"$, set $x$ to $x."left"$; otherwise, set $x$ to $x."right"$.
4. Set $z."parent"$ to $y$.
5. If $y$ is null, then the tree was empty and $z$ becomes the root of the tree.
6. Otherwise, if $z."key" < y."key"$, set $y."left"$ to $z$; otherwise, set $y."right"$ to $z$.

#pseudocode-list()[
  #smallcaps()[BST-Insert(T, z)]:
  + y = null
  + x = T.root
  + while x != null:
    + y = x
...
- 0 children: remove $x$
- 1 child: splice x.
- 2 children: find successor $y = "node"$ with smalled $"key" > x."key"$. Splicy y and replace x with y.

#pseudocode-list()[
  #smallcaps()[BST-Delete(T, z)]:
  + if z.left == null:
    + Transplant(T, z, z.right)
  + else if z.right == null:
    + Transplant(T, z, z.left)
  + else:
    + y = Tree-Minimum(z.right) // successor of z
    + if y.parent != z:
...
#definition(title: "2-3 tree")[
  - Rooted tree
  - each internal node has either 2 or 3 children
    - 2-node: 2 children, 1 key
    - 3-node: 3 children, 2 keys
  - each leaf has 1 or 2 keys

  *properties*:
  - Symmetric order
    - Inorder traversal of the tree yields keys in sorted order.
  - Perfectly balanced
    - every path from the root to a leaf has the same lenght
  - height of tree is $Theta(log n)$
]

=== Search in a 2-3 tree
To search for a key $k$ in a 2-3 tree:
1. Start at the root node.
2. If the current node is a leaf, check if it contains the key $k$. If it does, return the associated data; otherwise, return null.
3. If the current node is a 2-node with key $k_1$:
  a. If $k < k_1$, move to the left child.
  b. If $k > k_1$, move to the right child.
  c. If $k = k_1$, return the associated data.
4. If the current node is a 3-node with keys $k_1$ and $k_2$:
  a. If $k < k_1$, move to the left child.
  b. If $k_1 < k < k_2$, move to the middle child.
  c. If $k > k_2$, move to the right child.
  d. If $k = k_1$ or $k = k_2$, return the associated data.
5. Repeat steps 2-4 until a leaf is reached or the key is found.

#pseudocode-list()[
  #smallcaps()[TwoThree-Search(T, k)]:
  + x = T.root
  + while x is not null:
    + if x is a leaf:
      + if k in x.keys:
        + return x.data associated with k
      + else:
        + return null
    + else if x is a 2-node with key k1:
      + if k < k1:
        + x = x.left
      + else if k > k1:
        + x = x.right
      + else: // k == k1
        + return data associated with k1
    + else if x is a 3-node with keys k1 and k2:
      + if k < k1:
        + x = x.left
      + else if k1 < k < k2:
        + x = x.middle
      + else if k > k2:
        + x = x.right
      + else: // k == k1 or k == k2
        + return data associated with the matching key
]

=== Insert in a 2-3 tree
To insert a key $k$ into a 2-3 tree:
1. Search for $x$
2. add $x$ at leaf node
- if the code is too large, split it and move the middle key to the parent. repeat if necessary.

#pseudocode-list()[
  #smallcaps()[TwoThree-Insert(T, k)]:
  + z = new node with key k
  + y = null
  + x = T.root
  + while x is not null:
    + y = x
